...
export default function RoleScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg, padding: 24, paddingTop: 80 }}>
      ...
      {[
        { id: 'hirer', icon: 'star-outline', title: "I'm a Client", desc: 'Book beauty & grooming services' },
        { id: 'provider', icon: 'briefcase-outline', title: "I'm a Provider", desc: 'Offer your beauty & grooming services' },
      ].map(r => (
        <TouchableOpacity key={r.id} onPress={() => setRole(r.id)}
          style={{ flexDirection: 'row', alignItems: 'center', gap: 16, borderRadius: 16, borderWidth: 1.5, borderColor: role === r.id ? C.primary : C.border, padding: 16, marginBottom: 12, backgroundColor: role === r.id ? C.bg2 : C.white }}>
          <MCI name={r.icon as any} size={32} color={role === r.id ? C.primary : C.text2} />
          <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity onPress={() => { if (role === 'provider') router.replace('/(provider)/onboarding'); else router.replace('/(hirer)/(tabs)'); }}
        disabled={!role}
